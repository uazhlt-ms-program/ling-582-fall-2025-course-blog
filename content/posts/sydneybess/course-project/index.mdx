...
First, I wanted to see how the model did with just base-printed and no special training or adding of special tokens. I have more about how the model actually performed in the error analysis, but here is a sample of a baseline model.
...
OCR Prediction:
KUN XUX 11/8, SUBGAGI TOTOS

[Levenshtein Distance](https://planetcalc.com/1721/): 26 character edits 
[CER](https://taylorarchibald.com/cer/): 50.00% which is less than random which is good.
[WER](https://taylorarchibald.com/cer/): 125%

You'll see the comparison soon, but now let's talk about the results of the model.
...
For this I used the final checkpoint in the model, because it seemed to have more accurate results then what was saved for lowest C.E.R.
...
[CER](https://taylorarchibald.com/cer/): 10.71%
[WER](https://taylorarchibald.com/cer/): 25%

![Differences](./images/real_1_result.png)
While it isn’t as low a CER as the model said, it isn’t too bad. As we’ll see, it tends to interpret s as either c’s, s’s, or u’s. The word error rate makes sense because even in training the model, it was higher than C.E.R because it's harder to ace words (especially long ones) than it is to get most characters right.

Compared to the baseline prediction I mentioned in the previous section, having a model that can process the special characters does wonders for the character error rate.

The delta for this is:
| Metric | Baseline | New Model | Delta (New - Base) |
|--------|----------|-----------|----------------|
| **Levenshtein Distance** | 26 edits | 3 edits | **-23 edits** |
| **CER** | 50.00% | 10.71% | **-39.29%** |
| **WER** | 125% | 25% | **-100%** |
...
Actual Transcription:
tcil:dju'sɛnts kuḿ tćaḿ äya'a guĺ:nt'a'q́ʷsus. tätc
...
![Differences](./images/new_3_reference.png)

The ‘dj’ pattern is very rare, probably occurring only once or twice, which is why it couldn’t be predicted. Again, it confuses s’s and c’s.

Another difference is how thick and uneven the font is compared to the other samples, and thus it does poorer. This suggests the state of the image matters.
...
[WER](https://taylorarchibald.com/cer/): 62% Not that good.
...
For this, it seemed to completely miss words at the beginning and end, and to mess up most words in between. The words it did generate are words found in the generated words and ground truth words, so it can't do any inference on new words (let alone even some concrete words)
...
OCR Prediction:
hɔi tci''ɫ tc:ci'pɛnts. kuḿ ɫa sm:yi'w. hɔi

[Levenshtein Distance](https://planetcalc.com/1721/): 42 character edits 
[CER](https://taylorarchibald.com/cer/): 76.60% 
[WER](https://taylorarchibald.com/cer/):87.50%

![Differences](./images/finetune_3.png)

Here it gets the correct length of the sentence, but misses most words completely, with a 76% C.E.R (which is technically better then the model gave)
...
Once trained and the special tokens are added it does pretty well. It does have more errors though on longer words, and words that are completely unseen. There are a few character's like 's' that is misread, or because of the amount of a's in the dataset, a's are filled in for characters that are tough to tell apart (as if it is default).
...
[README.md](https://github.com/uazhlt-ms-program/ling-582-fall-2025-course-project-code-indigenous-language-ocr/blob/main/README.md) I demonstrated reproducibility and set up file seeds, along with how to run the model.
