...
Below are the results of my best training run. The training loss, logged every 10 batches can be seen on the left, while the validation loss by epoch on a 20% holdout set is shown on the right. These show that the loss never drops below ~0.9. That is high considering that it is MSE. 
...
Unfortunately, upon further evaluation, it is obvious that the model has simply learned to predict the mean:
<div style="display:flex; align-items:center; flex-direction: column">
...
I did some limited error analysis to confirm that the model learned to predict the mean.
...
julia> println("MAE: $mae, RMSE: $rmse, MAPE: $mape%")
MAE: 0.5873926876750887, RMSE: 0.9436236172835178, MAPE: 229.95062746555342%
```
<div style="display:flex; align-items:center; flex-direction: column">
